...
internal class ShoppingDataSource {
    ...
    suspend fun addItems(liveId: String, items: List<QItem>) {
        ...
        QLiveHttpService.post("/client/item/add", jsonObj.toString(), Any::class.java)
    }
    ...
    suspend fun deleteItems(liveId: String, itemIDList: List<String>) {
        ...
        jsonObj.put("live_id", liveId)
        jsonObj.put("items", jsonArray)
        QLiveHttpService.post("/client/item/delete", jsonObj.toString(), Any::class.java)
    }
    ...
    suspend fun updateStatus(liveId: String, items: HashMap<String, Int>) {
        ...
        jsonObj.put("items", jsonArray)
        QLiveHttpService.post("/client/item/status", jsonObj.toString(), Any::class.java)
    }
    ...
    suspend fun getItemList(liveId: String): List<QItem> {
        ...
        return QLiveHttpService.get("/client/item/$liveId", null, null, p)
    }
    ...
    suspend fun setExplaining(liveId: String, ItemID: String) {
        QLiveHttpService.post("/client/item/demonstrate/${liveId}/${ItemID}", "{}", Any::class.java)
    }

    suspend fun cancelExplaining(liveId: String) {
        QLiveHttpService.delete("/client/item/demonstrate/${liveId}", "{}", Any::class.java)
    }

    suspend fun getExplaining(liveId: String): QItem {
        return QLiveHttpService.get("/client/item/demonstrate/${liveId}", null, QItem::class.java)
    }

    //跟新单个商品顺序
    suspend fun changeSingleOrder(live_id: String, item_id: String, from: Int, to: Int) {
        QLiveHttpService.post("/client/item/order/single", JSONObject().apply {
            put("live_id", live_id)
            put("item_id", item_id)
            put("from", from)
            put("to", to)
    ...
    suspend fun changeOrder(live_id: String, orders: HashMap<String, Int>) {
        QLiveHttpService.post("/client/item/order", JSONObject().apply {
            put("live_id", live_id)
            put("items", orders)
        }.toString(), Any::class.java)
    }
    ...
    suspend fun deleteRecord(liveId: String, recordIds: List<Int>) {
        QLiveHttpService.post("/client/item/demonstrate/record/delete", JSONObject().apply {
            put("live_id", liveId)
